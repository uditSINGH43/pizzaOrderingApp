...
fun PizzaSelectionScreen(
    ...
    var selectedPizzaidx by remember {
        mutableStateOf(-1)
    }
    Box(
        modifier = modifier
        ...
        LazyColumn(
            ...
            itemsIndexed(dummyPizzaRepository()) { idx, it ->
                PizzaItem(pizza = it) {
                    dummyPizzaRepository().indexOf(it).let { itemIdx ->
        ...
        AnimatedVisibility(selectedPizza != null) {
            Row(
                ...
                Text(
                    text = "Selected Pizza: ${selectedPizza}",
                )
                Spacer(modifier = Modifier.weight(.5f))
                IconButton(onClick = { selectedPizza = null }) {
                    Icon(imageVector = Icons.Default.Close, contentDescription = "Done")
